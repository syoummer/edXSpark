...
def simpleTokenize(string):
    ...
    """
    return [i for i in re.split(split_regex, string.lower()) if len(i) > 0]
...
def tf(tokens):
    ...
    for token in tokens:
        if token in dictionary:
            dictionary[token] += float(1.0)
        else:
            dictionary[token] = float(1.0)
    length = len(tokens)
    for word in dictionary.keys():
        dictionary[word] /= float(length)
    #print length
    return dictionary
...
def idfs(corpus):
    ...
    """
    N = corpus.count()
    uniqueTokens = corpus.flatMap(lambda x: list(set(x[1])))#.distinct()
...
def tfidf(tokens, idfs):
    ...
    tfs = tf(tokens)
    #print idfs
    tfIdfDict = {key: (value*float(idfs[key])) for (key, value) in tfs.iteritems()}
    return tfIdfDict
...
def dotprod(a, b):
    ...
    """
    #return <FILL IN>
    multiply = [(v1*v2) for (k1, v1) in a.items() for (k2, v2) in b.items() if (k1 == k2)]
    return 0 if len(multiply) == 0 else reduce(lambda x, y: x + y, multiply)
...
def norm(a):
    ...
    return 0 if len(a) == 0 else math.sqrt(reduce(lambda x, y: float(x) + y, map(lambda (k,v): float(v)*v, a.items())))
...
avgSimDups = (trueDupsRDD.map(lambda (k,v): v[0]).reduce(lambda x, y: x + y))/float(trueDupsCount)

nonDupsRDD = (sims
              .leftOuterJoin(goldStandard)
              .filter(lambda (k, v): v[1] == None))
avgSimNon = nonDupsRDD.map(lambda (k, v): v[0]).reduce(lambda x, y: x + y) / float(nonDupsRDD.count())
...
def invert(record):
    ...
    ID = record[0]
    tokens = record[1]
    pairs = map(lambda k: (k, ID), tokens.keys())
    return (pairs)
...
def fastCosineSimilarity(record):
    ...
    s = 0
    v1 = amazonWeightsBroadcast.value[amazonRec] 
    v2 = googleWeightsBroadcast.value[googleRec] 
    for k in v1.keys():
        if (k in v2):
            s += v1[k] * v2[k]
