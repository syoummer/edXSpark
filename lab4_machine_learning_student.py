...
def getCountsAndAverages(IDandRatingsTuple):
    ...
    avgRating = avgRating / float(numOfRatings)
    return (movieID, (numOfRatings, avgRating))
...
movieNameWithAvgRatingsRDD = (moviesRDD
                              .join(movieIDsWithAvgRatingsRDD)).map(lambda (x, (a, (count, rating))): (rating, a, count))
...
movieLimitedAndSortedByRatingRDD = (movieNameWithAvgRatingsRDD
                                    .filter(lambda (rating, name, count): count > 500)
...
def computeError(predictedRDD, actualRDD):
    ...
    squaredErrorsRDD = (predictedReformattedRDD
                        .join(actualReformattedRDD).map(lambda (x, (left, right)): (left - right) * (left - right)))
...
myRatedMoviesIds = [z for (x, y, z) in myRatedMovies]
myUnratedMoviesRDD = (moviesRDD.map(lambda (id_, title_): (myUserID, id_))
                      .filter(lambda (id_, id2_): id2_ not in myRatedMoviesIds))
...
ratingsWithNamesRDD = (predictedWithCountsRDD
                       .join(moviesRDD).map(lambda (id, ((rating, count), name)): (rating, name, count))
